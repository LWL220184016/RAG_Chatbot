...
class TranscriptionConfig:
    ...
    pretrained_name: Optional[str] = "nvidia/parakeet-tdt-1.1b"  # Name of a pretrained model
    audio_dir: Optional[str] = "./"  # Path to a directory which contains audio files
    dataset_manifest: Optional[str] = None  # Path to dataset's JSON manifest

    # General configs
    output_filename: Optional[str] = "./output.json"
    batch_size: int = 64
    num_workers: int = 10
    ...
    # Chunked configs
    chunk_len_in_secs: float = 4  # Chunk length in seconds
    total_buffer_in_secs: float = 100.0  # Length of buffer (chunk + left and right padding) in seconds
    model_stride: int = (
